...
// TODO - Check with Guillaume G when asciidoctor revealjs backends will be ready
// require('asciidoctor.js/dist/npm/asciidoctor-factory.js')
// require('asciidoctor.js/dist/npm/asciidoctor-slide.js')
// require('asciidoctor.js/dist/npm/asciidoctor-revealjs.js')
...
function initProject (type, force, dir) {
  ...
  switch (type) {
    ...
    case 'slideshow':
      ...
      repos = { 'revealjs': 'https://codeload.github.com/hakimel/reveal.js/tar.gz/3.3.0',
  ...
  if (type === 'slideshow') {
    ...
    async.series([
      // TODO - Add Clean
      function (callback) { builder.downloadDependencies(callback); }
    ], function () {
...
Builder.prototype.downloadDependencies = function (callback) {
  ...
  async.series([
    function (callback) { builder.getContentFromURL(repos.revealjs, path.join(projectDir, '_temp', 'revealjs.tar.gz'), callback); }, // download revealjs project
    function (callback) { builder.getContentFromURL(repos.revealjsbackend, path.join(projectDir, '_temp', 'revealjs-backend.zip'), callback); }, // download revealjs backends
    function (callback) { builder.untar(path.join(projectDir, '_temp', 'revealjs.tar.gz'), 'revealjs', path.join(projectDir, '_temp'), callback); },
    function (callback) { builder.untar(path.join(projectDir, '_temp', 'revealjs-backend.zip'), 'revealjs-backend', path.join(projectDir, '_temp'), callback); },
    function (callback) { copyDir(path.join(projectDir, '_temp', 'revealjs'), path.join(projectDir, 'generated', 'reveal.js')); callback(); },
    function (callback) { fs.rename(path.join(projectDir, '_temp', 'revealjs-backend', 'templates', 'jade'), path.join(projectDir, '_temp', 'revealjs-backend', 'revealjs')); callback(); }
  ], function () {
    typeof callback === 'function' && callback();
  });
};
  ...
  if (cfg) {
    if (fs.statSync(cfg.source).isDirectory()) {
      ...
      glob(pattern, {}, function (err, files) {
        ...
        for (var i = 0; i < files.length; i++) {
          console.log(files[i]);
          var toFile = path.join(cfg.destination, getFileNameWithoutExtension(files[i]) + '.pdf');
