...
function initProject(type,force,dir) {
    ...
    switch(type) {
        ...
        case 'slideshow':
            ...
            break;
        default:
            templates = ["config.yaml","simple.adoc"];
    }
    ...
    // Create the source & generated directories
    mkDir(path.join(dir.toString(),'source'));
    mkDir(path.join(dir.toString(),'generated'));

    // Copy the config(s) to the project directory
    for (var i = 0, len = configs.length; i < len; i++) {
        let configPath = path.join(getTemplatesDir(),configs[i]);
        copyFile(configPath,dir.toString());
        log.info("Copy the yaml config file : " + config[i].toString() + " to the project");
    }

    // Copy the asciidoctor template(s) to the project directory
    for (var i = 0, len = templates.length; i < len; i++) {
        let templatePath = path.join(getTemplatesDir(),templates[i]);
        copyFile(templatePath,dir.toString());
...
function convertToPdf(cfg) {
    var options = {
        ...
        "header": {
            "height": "5mm",
            "contents": {
                default: '<div style="color: #444; font-size: 80%; text-align: center;">Author: Microservices</div>'
            }
        },
        "footer": {
            "height": "10mm",
            "contents": {
                ...
                default: '<div style="color: #444; font-size: 80%; text-align: center;">{{page}}/{{pages}} - &copy; Red Hat - 2016</div>', // fallback value
                last: 'Last Page'
            }
        },
        // File options
        ...
        "phantomPath": "/Users/chmoulli/MyProjects/litoria/node_modules/phantomjs-prebuilt/bin/phantomjs"
    };
    ...
    if (cfg) {
        if (fs.statSync(cfg.source).isDirectory()) {
            ...
            var to_file = path.join(cfg.destination, getFileNameWithoutExtension(cfg.source) + ".pdf");
            var imagePath = path.join(process.cwd(), path.dirname(cfg.source));
            log.debug("Image path : " + imagePath);

            var html = fs.readFileSync(cfg.source, 'utf8');
            ...
            //var newHtml = html.replace("img src\"","img src=\"//" + imagePath + "/");
...
function copyFile(srcPath, targetDir) {
    var source = path.basename(srcPath);
    var target = path.join(targetDir,srcPath);
    log.info("From: " + source + " , to : " + target);
