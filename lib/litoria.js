...
function initProject(force,dir) {
    ...
    mkDir(path.join(dir.toString(),'_git_repos'));
}
...
function convertToHtml(cfg_file) {
    ...
    if (cfg) {
        if (fs.statSync(cfg.source).isDirectory()) {
            ...
            */
            glob(cfg.source + '/' + extension, function (err, files) {
                if(!cfg.options['to_file']) {
                    cfg.options['to_file'] = getFileNameWithoutExtension(f) + ".html";
                }
                ...
                files.each(function(file) {
                    processor.$convert(file, options);
                });
            })
        } else {
            ...
            var attrs = opal.hash(cfg.attributes);
            options["$[]="]("attributes",attrs);
            processor.$convert(cfg.source, options);
        }
    }
}
